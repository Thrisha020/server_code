...
async def repository_manager(user_in: AutomationInput, db=Depends(get_db)):
    try:
        ...
        #  decides which tool from langchain_tools 
        #user_prompt = f"{base_prompt}\nUser Query: {user_in.query}"
        try:
            response = agent_executor.run(user_in.query)
        #Executes the selected tool's function
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("custom_api:app", host="172.17.0.8", port=8000,workers = 2)
